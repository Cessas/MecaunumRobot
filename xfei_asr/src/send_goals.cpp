...
tts2voice_pub_=n_.advertise<std_msgs::String>("/voice" ,1);
...
p[i] = goal[i];
    ...
    std_msgs::String error2_msg;

    std::stringstream ss1;
    std::stringstream ss2;
    ss1 << "对不起，我没明白你的意思，我草你妈 " ;
    ss2 << "很高兴为您服务" ;
    error2_msg.data = ss2.str();
...
if(p[4]=='b'&&p[5]=='d'&&p[6]=='d') secondgoal=bedroomgoal;
...
ac.waitForResult();
  if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
    tts2voice_pub_.publish(error2_msg);
}
}
